...
- `__init__.py` — `GlobalPlugin` class, all trigger logic, and the `Z` script (browse-mode binding). Hooks **`event_documentLoadComplete` ONLY** — explicitly NOT `event_gainFocus`, because that fires on every alt-tab and in-document focus change.
  ...
  **Read this before touching the trigger (2026-07-14, verified in NVDA source by two independent reviews).** Two things this file used to claim are FALSE, and believing them cost the add-on two thirds of its page loads:
  ...
  Debounce gates, in order. Each is a reason to do nothing:

  - (a) **Same TI AND same URL: skip.** This is the document-identity gate. An iframe or ad load fires its own `documentLoadComplete` but resolves through containment to the MAIN page's TI and URL, so it lands here correctly, as do duplicate double-fires. Accepted gap: a genuine re-navigation to the IDENTICAL URL on a reused TI (F5, a form POST returning the same URL) is skipped. `Z` covers it, and separating it from a double-fire would need a document-generation signal NVDA does not expose.
  - (b) **Same URL within `_REFIRE_COOLDOWN_SEC` (2.0 s): skip.** Catches SPA-ish sites like DuckDuckGo and Gmail that emit duplicate document-load events for one logical page load.
  - (c) **Post-landing suppression, same URL as the last SUCCESSFUL landing within `_LANDED_SUPPRESS_SEC` (120 s): skip.** SPA re-renders fire fresh events with new TIs well past the 2 s cooldown, and the re-rendered tree is often WORSE than the one already landed on (Zoom collapsed its description into accordions), so re-detecting yanked the user off a good landing into mid-form. One page visit, one landing. `_recordLanding` stamps the URL on both success paths.
  - (d) **Focus is on an editable control: skip silently**, per guardrail #6.
  - (e) **Restored-position gate** (`_caretIsMidPage` plus `_seenUrls`): browse cursor past the first character AND (the URL was already visited this session OR the URL carries a real anchor fragment, not a `#/` SPA route). BOTH signals are required. Caret-mid-page alone misfires on first visits to pages that initialize the caret below the top, and Back navigation by definition returns to a seen URL. `_seenUrls` is a session-scoped URL-to-timestamp dict, membership checked BEFORE recording at PROCEEDING time, pruned to the newest 250 at 500 entries.

  `Z` bypasses (a), (b), (c) and the URL/timestamp gates; double-Z on an excluded site bypasses (e) as well, an explicit request being explicit. The retry uses a DIFFERENT check from (e): it captures the caret at scheduling time and abandons only if the caret MOVED during the wait, because comparing against document-top would permanently break retries on those same below-top-caret pages.

  Two trade-offs, both accepted. An F5 within 120 s of a landing gets no auto-landing, and `Z` covers it. And alt-tab or a tab switch NEVER auto-triggers, because alt-tab produces no `documentLoadComplete` at all (it is a browser *load* event, mapped from `IA2_EVENT_DOCUMENT_LOAD_COMPLETE`), so quiet tab switches are structurally safe rather than dependent on any gate. Casey confirmed 2026-07-06 that this is the DESIRED behavior. Do not "fix" it by hooking `event_gainFocus`.

  **Readiness poll.** `documentLoadComplete` means the DOM finished loading, which is NOT when NVDA finishes building the virtual buffer, and NVDA only builds a TI in its pre-step when the loaded object is the focus or a focus ancestor (`eventHandler.py:431-432`). So the TI can arrive unready OR as `None`, and both are polled, 250 ms by 12 attempts. The `None` case is gated by a URL-scheme allowlist (http/https/file), the same trick NVDA itself uses to tell web documents from email (`browseMode.py:2406-2413`): mail clients expose `imap:`/`mailbox:`/`news:` URLs, so this keeps us out of Thunderbird message previews without hard-coding an app name. **Beware:** on a REUSED TI, `isReady` stays True throughout the buffer's re-render, so the poll never engages and detection can walk a buffer still holding the PREVIOUS page. The stale-position guard is what catches that.

  **Flow.** `feedback.working()`, then `feedback.progressStart()`, detection in `_runDetection`, dispatch by intent in `_handleResult`, `feedback.progressStop()` in a finally.

  **Speech path differs by intent.** ARTICLE, LIST, NOTICE and KEY_RESULT land via `speech.speakTextInfo` on a captured textInfo, with `speech.cancelSpeech` immediately before it, which is the only sanctioned cancel point. FORM splits by page shape. A BARE form (no substantial preamble) gets `ui.message` announcing the form title, mode-agnostic so it survives NVDA's auto-switch into focus mode when the page auto-focuses an input, and then `setFocusOnFirstFormInput` moves keyboard focus to the first field so NVDA's own focus speech announces it; without that, Google-Forms-style pages swallow a browse-mode `speakTextInfo` silently. A RICH-PREAMBLE form (`web.formWantsBrowseLanding`: any non-chrome paragraph of 200 chars or more) instead gets a normal browse-mode landing on the form title, because focusing the first input on a Zoom-webinar-registration page skipped the user past about 2000 characters of description.

  **Retry mechanism.** If a detection attempt returns False (no landing acted on), another attempt is scheduled via `wx.CallLater`, bounded by `_MAX_DETECTION_ATTEMPTS` (3 including the first). The delay depends on WHY the attempt failed. A tree that yielded real nodes and still found no landing has genuinely answered, so it gets exactly ONE more look at `_RETRY_DELAY_MS` (1500 ms) and no third; re-walking such a page costs seconds on exactly the heavy pages that can least afford it. An UNBUILT tree is nearly free to re-walk, so it earns a third attempt at `_EMPTY_RETRY_DELAY_MS` (1250 ms) each, giving three looks at 0, 1.25 and 2.5 seconds. Retries are silent (no second working tone) and verify the URL has not changed. An ACCEPTED navigation cancels a pending retry; a `Z` press deliberately does NOT (if Z moved the caret the retry abandons on its own caret-moved check, and if Z found nothing the page is usually still hydrating, which is what the retry waits for). Both the retry and the readiness poll abandon when keyboard focus is in a DIFFERENT browse-mode document (`_focusIsOnAnotherDocument`, 2026-09-24): a tab switch fires no load event, so nothing else stops page A's callback from speaking or moving focus while the user reads tab B. Focus with no tree interceptor at all does not count as elsewhere, because that is also the ordinary state while a page comes up. The ready-TI path also skips documents whose URL names a mail-client scheme (`_isMailDocument`), a denylist rather than the poll path's allowlist so an unreadable URL or a reader-view page is still detected. `notFound` plays only on the LAST attempt's no-result outcome, never the first, so the early silence covers the wait cleanly. This is the principled fix for SPA hydration delay: no per-site code, just wait and look again.

  **Stale-position guard (load-bearing, do not remove).** We capture a TextInfo per node during the walk and speak it afterwards, but the walk takes 1.5 to 2 seconds and a hydrating page rebuilds the virtual buffer DURING that window, so positions captured early are stale by the time the walk ends. The add-on was speaking a different paragraph than the one it landed on, 7 of 42 landings in an 83-page soak, and the classifier's choice was frequently CORRECT. It also explains silent landings: a stale position that expands to an empty range makes `speakTextInfo` neither raise nor speak. So before moving the caret or speaking, `_handleResult` re-expands the captured position and checks it still holds the paragraph the classifier chose (`web.landingTextMatches`); on drift it re-anchors BY TEXT via `tsMod.findLandingByText`, which is NVDA's `TextInfo.find` (returns bool, repositions to the match start). A retry is NOT a substitute, and believing it was cost a whole build: the buffer drifts DURING the walk, so a re-walk races the same way. Text is the stable anchor; the offset is not. If the re-find also fails we stay silent, because reading the WRONG paragraph is worse than reading none. Full account, including the intermediate build that turned wrong-text into silence: `implementation-notes.md`, entry "2026-07-14 (rest of the day)".
...
**What every field on those lines MEANS, and which ones are traps, is in DEBUGGING.md, "Reading the diagnostic lines field by field".** It is kept there rather than here because that is the file you are in when you are actually reading a log. Two other diagnostic lines exist and are documented in the same place: `[TMTS walk-phase]`, which breaks the walk's wall clock down by CALL SITE and is the first thing to read on an unexplained slow walk, and `[TMTS walk-preamble-drops]`, which lists what the scope filter dropped BEFORE the scoped region started.
...
- **Persistent perf log** (`TextMarksTheSpot-perf.log` in NVDA's user config folder, which is `%APPDATA%\nvda` for an installed copy; `treeSummary._diagDir()` asks NVDA via `globalVars.appArgs.configPath` so a portable copy uses its own `userConfig`) — append-only, ISO-timestamped, survives NVDA and Windows restarts. Self-rotates at 1 MB to `TextMarksTheSpot-perf.log.old` (one generation of history kept). Use this when collecting data across multiple sessions or asking a fresh AI session to analyze patterns. Write errors are swallowed silently so a locked or unwritable file can never break detection.

**BOTH PERSISTENT LOGS ARE OPT-IN (2026-07-22) AND THAT IS A PRIVACY GATE, NOT A TIDINESS ONE.** The perf log and the capture log each record the FULL url of every page detected, query string and all, and the perf log outlives NVDA restarts, so it accrues for months. One real day of them banked an invoice link, OAuth authorization codes, and an app path carrying a `client_secret`. They were originally gated on NVDA's DEBUG log level, which is NOT consent: people raise the log level for unrelated reasons and would never guess a screen-reader add-on had started recording where they go. Both now write only when `TextMarksTheSpot-diagnostics-enabled` exists in NVDA's user config folder AND NVDA itself permits disk writes (`NVDAState.shouldWriteToDisk`, so never on a secure desktop or from the launcher), checked once per session by `treeSummary._diagnosticsEnabled()` so it cannot flip on under a user mid-browse; an unreadable folder reads as OFF. A log whose rotation keeps failing stops growing at twice its cap (`_rotateOrRefuse`). Pinned by `tests/test_diagnostic_optin.py` and four separate entries in `tests/sabotage_check.py`, each writer sabotaged independently, because a suite watching only one would not notice the other reverting to DEBUG.

**The session-log copy was ALSO gated on 2026-08-18, reversing the earlier decision that it could stay open.** The old reasoning was that NVDA's own log is one the user asked for and is wiped on the second restart. What settled it the other way: NVDA's default log level is `INFO` and it logs spoken text at the IO level, so our lines only ever appeared for someone who had raised the level, which is precisely the "a debug log level is not consent" case; and at that level NVDA is already logging every phrase it speaks, so we were never the dominant source of page content in such a log, but we were much the tidiest, one clean greppable `url=` per page load. Volume was never the risk. Structure and persistence are. Full argument: `implementation-notes.md`, entry "2026-08-18 - the session-log copy is gated too; the add-on is no one's browsing record".
...
python -m ruff check . && python -m ruff format --check .   # lint + format (tabs, 110 cols)
```
...
**Lint and format are BOTH gated, and ruff is PINNED to the version in `.github/workflows/tests.yml`.** The pin is not fussiness: an unpinned install put ruff 0.16.7 against a tree that 0.14.5 called clean and drew 218 errors, so without it some unrelated future push fails on code its author never touched. Bumping the pin is a deliberate act that comes with its own diff to clean up. The tree was reformatted in one commit on 2026-09-10, listed in `.git-blame-ignore-revs` so it does not sit on top of the reasoning history that lives in this codebase's comments; run `git config blame.ignoreRevsFile .git-blame-ignore-revs` once on a fresh clone to get the same locally (GitHub honors the file on its own). Two things about the ruff config are worth knowing before touching it. `probes` is excluded, because those are frozen throwaway add-ons still calling the pre-2026-09-06 snake_case API. And `format.line-ending` is `auto` rather than `lf`, because `.gitattributes` sets `* text=auto` so git already stores LF regardless of the working tree, and pinning ruff to `lf` on a CRLF Windows checkout made `ruff format --check` report every file in the repository.
...
out of git), `test_regex_linear.py` (the full-text detectors stay linear on hostile
text AND agree with the old patterns), `test_config_hosts.py` (canonical hostname
matching), and `test_plugin_guards.py`, which drives the GlobalPlugin itself through
the fakes in `tests/nvda_stubs.py`. That harness is the only unit-level coverage
`__init__.py` has; it proves decisions, not real NVDA behaviour, which stays with
the VM.

**Every detector that runs on a chunk's FULL text must be linear time** (security
audit, 2026-09-24). Those run on NVDA's main thread inside one regex call the walk
deadline cannot interrupt, and a web page controls the text: two quadratic patterns
let a 40,000-space paragraph freeze NVDA for five seconds, reproduced on the VM.
When adding one, avoid adjacent unbounded runs that can split the same characters
...
- `Z` in browse mode scans forward from the user's current browse cursor to the next substantial content paragraph. It skips headings because NVDA's `H` already handles heading navigation, and it skips known chrome paragraphs. If nothing qualifies below the cursor, it says "Nothing else to land on" and does not wrap. On a page too long for one 2 s walk (the walk always starts at the top), a TRUNCATED first walk with nothing below the cursor triggers a second walk from the cursor (`buildTreeSummary(ti, startInfo=caret)`, 2026-09-24); before that, Z said "Nothing else" past roughly the thousandth paragraph. The second pass never lands from a tree whose scope fell back (`scopeFellBack`), and its short-content fallback is decided by the first walk, since a partial tree cannot know the whole page has no 50-char paragraph. Known and harmless: a walk started mid-document can spin at the document end until the node cap and report itself truncated (seen on the VM, about 50 ms); nothing on the Z path reads that flag.
- `Shift+Z` returns to the saved automatic landing on the current page without recalculating. If NO landing is saved for the page (the auto-trigger structurally can't fire on tab switches — no `documentLoadComplete` — so a stale tab has nothing saved), Shift+Z runs full one-time detection instead, resetting the debounce gates like Z does; on an excluded site it keeps the plain "No saved landing" message (double-Z stays the only exclusion override). Added 2026-07-16 after the starttesting.net login tab left the user stranded with no gesture that could summon detection. Since 2026-09-24 a saved landing is VERIFIED before use (`_verifiedSavedLanding`): re-expand and compare against the saved MainNode, re-find by text if the page re-rendered or this is a different tab on the same URL, and run fresh detection rather than read a paragraph that is not the landing. It uses the same `landingTextMatches` / `findLandingByText` pair as the auto-landing's stale-position guard, so the two cannot disagree.
...
**The `chrome` case is positional too (2026-07-18, `scope=chrome-pos`), and two design attempts died before the shipped one.** With no `<main>` there is no range to be INSIDE, so it inverts: `_findMainLandmark` collects the CHROME landmark ranges in the pass it already makes, and the walk drops any chunk whose START falls inside one. It was measured before it was built (DEBUGGING.md): the parent chains really were about 90 percent of the walk on the page that prompted it.

**Dead design, preserved on branch `chrome-pos-attempt`: gate on "did the landmark enumeration complete?" It is UNIMPLEMENTABLE, not merely rejected.** NVDA's `VirtualBuffer._iterNodesByAttribs` wraps its `VBuf_findNodeByAttributes` call in a handler that DISCARDS the exception and returns, confirmed by disassembling the installed `virtualBuffers/__init__.pyc`. A native failure is therefore indistinguishable from natural exhaustion, and a completeness flag can return True holding a partial list. **This one fact also killed `chrome-none`, so check any new design against it before writing code:** a design that asks whether a scan finished cannot work.

**What shipped instead: bounded trust.** Never ask whether the scan finished, only how far it got. `trustBoundary` is the START of the last landmark successfully placed, `_startsBefore` compares STRICTLY, and `trustFrozen` is monotone, so if the boundary landmark was placed then every earlier emitted landmark was too. Truncation then costs speed, never correctness.

**That was still not enough, because the probe that licensed it was hollow.** A passive probe reported `ordered=True` on 15 of 15 real pages and was presented as evidence. NVDA seeds each landmark search with the PREVIOUS match's start offset, so nondecreasing starts were guaranteed a priori and `ordered=False` could never have occurred. The property actually needed was COMPLETENESS: a landmark starting at the SAME offset as the one just returned can be silently skipped, `<section aria-label="...">` around a `<nav>` with no text between them being exactly that shape, and the consequence is a blind user landing in a menu. **When a probe comes back unanimous, ask whether it could ever have come back the other way.**

**Soundness rests on a fact about NVDA's C++**, verified in `nvdaHelper/vbufBase/storage.cpp`: `nextNodeInTree` FORWARD is a PRE-ORDER walk, so an ancestor is always emitted before its descendants, and `findNodeByAttributes` reseeds through the TEXT LEAF at the match offset. Everything skipped is therefore a DESCENDANT of an emitted landmark, and the dangerous inverse (inner emitted, outer skipped and extending past it) cannot occur. So `_chromePosVerdict` answers in three steps: inside an emitted CHROME range, excluded, because nested chrome inside chrome excludes the same text and the omission is harmless; inside an emitted NON-CHROME range (`untrustedRanges`), defer to the identity filter, which is the only place an omitted nested nav can hide; inside no emitted range, the positional answer is sound. Every tri-state `None` from `_startsInAny` or `_startsBefore` propagates to "ask identity". There is deliberately NO "assume not excluded" branch, because that is how navigation becomes article text.

**Test the CHAIN, not the pieces.** `_chromePosVerdict` and `_selectScope` are pure and unit-tested, and that was not enough: both reviewers showed that deleting the `otherRanges` append left the whole suite green, because every test hand-built its own untrusted list. `tests/test_chrome_scope.py` now drives scan to select to verdict end to end with no hand-built lists. Assume the next safety input is deletable without a test noticing unless the chain itself is tested.

**Two fixes from the same review, both independent of scoping.** (1) `setFocusOnFirstFormInput` filtered candidate fields against `<main>`, and with NO `<main>` that filter returned True for EVERY field, so "first form input in document order" meant the first editable element anywhere, routinely the header search box. That branch MOVES KEYBOARD FOCUS, so it dropped a blind user into site chrome. It now excludes fields starting inside a chrome landmark when there is no `<main>`. (2) In the `chrome-pos` path, a chunk whose `NVDAObjectAtStart` is None no longer defaults to in-scope. Elsewhere that default is harmless, but chrome-pos routes uncertain chunks to the identity filter AS its safety mechanism, and for an objectless chunk that filter cannot run at all. Those chunks are now out of scope and still reach `allNodes` so the depleted net can recover them; being wrong now costs a missed paragraph instead of a landing in a menu.

**The depleted net keys on `positionalDrops == 0` (positional EXCLUSIONS), not on the scope name.** Two earlier versions were wrong in opposite directions. Excluding `chrome-pos` outright switched the net off on the commonest no-`<main>` shape, because a single top-of-page nav puts the trust boundary at offset 0 and the page is chrome-scoped in all but name. Admitting `chrome-pos` unconditionally let a page whose exclusions WORKED be widened back open by two long chrome paragraphs, re-admitting the nav and cookie text that had been correctly removed. The scope name cannot separate those; the decision count can.

**A `chrome-none` scope existed briefly and was REMOVED 2026-07-19 as a merge blocker. Do not rebuild it, under any name.** It skipped chrome checking entirely on a page whose landmark enumeration yielded nothing, gated on `seen == 0 and exhausted and interactive_count > 0`. `exhausted` was doing the safety work and cannot: by the swallow above, a natively FAILED search arrives as `seen=0, exhausted=True`, byte-for-byte the shape of a genuinely landmark-free page. The interactive count does not corroborate either, since every quick-nav type runs through the same swallowing iterator. There is NO signal at the `LandmarkScan` layer that separates the two zero-seen cases; the per-chunk field stack is the only positive witness available. The cost of removal is real and accepted: on a landmark-free page with many chunks the walk returns to a roughly 14-dereference COM chain per chunk and may produce NO LANDING inside the 2 s budget. That is a silence, not a wrong landing, which is the direction guardrail 3 specifies. `tests/test_chrome_scope.py` pins the native-swallow shape and `tests/sabotage_check.py` fails when the shortcut returns, including under a new name and disguised as an empty exclusion list. One note on how it got in: the hazard had been looked at directly that same day and mis-filed as a pre-existing accepted risk. Noticing a hazard is not disposing of it.

Full histories for everything above, including the measurements and the review rounds: `implementation-notes.md`, entries "2026-07-18 - a wrong-answer cache, a regression from fixing it, and four review rounds", "2026-07-19 - MERGE BLOCKER in chrome-none, and the field-stack design survives review with conditions", and "2026-07-19 (later) - the chrome-none blocker is CLOSED by deletion".
