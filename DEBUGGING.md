...
- Persistent perf log: `TextMarksTheSpot-perf.log` in NVDA's user config
  folder (`%APPDATA%\nvda` for an installed copy, `userConfig` beside
  nvda.exe for a portable one; the add-on asks NVDA which).
  ...
  `article=/forms=/interactive=` counts, URL. Note the log field names are
  snake_case even though the Python attributes behind them are camelCase;
  they are a wire format shared with capture files already on disk.
  **Written ONLY when the opt-in marker file
  `TextMarksTheSpot-diagnostics-enabled` exists in that same folder, and
  never on a secure desktop, where NVDA forbids writes.** Raising
  NVDA's log level does nothing on its own, and neither does restarting; the
  marker is checked once per session, so the sequence is create the marker,
  restart NVDA, then reproduce. This gate covers the session-log copy too, so
  a live `NVDA+F1` diagnosis needs it as well. On a fresh dev machine, forget
  the marker and your own data silently stops arriving, which looks exactly
  like broken logging.
...
### Reading the diagnostic lines field by field

This is the reference for what each field on the `[TMTS perf]`, `[TMTS walk-phase]`
and `[TMTS walk-preamble-drops]` lines means, and which ones are traps. CLAUDE.md
carries the format example and the privacy rules; the meanings live here, because
this is the file you are in when you are actually reading a log.
...
`truncated=True` ALSO covers a walk an ERROR cut short (added 2026-09-24), and that case gets its own `[TMTS walk-interrupted]` line instead of `[TMTS walk-truncated]`. An error exactly AT the end of the document is how some buffers say "no more paragraphs" and is not flagged. Why it matters: `formWantsBrowseLanding` treats a truncated walk as "cannot prove there is no preamble" and takes the browse landing instead of moving focus, so a walk that died just after a form's title can no longer read as a bare form. Two Z-key lines exist since the same date: `[TMTS] Z: first walk truncated, second walk from caret nodes=N next=I` means the page was too long for one walk and Z looked again from the cursor. A second walk started mid-document can spin at the document end until the node cap and so reports `truncated=True` with a small `raw_seen`; that is harmless, since the Z path never reads the flag. `[TMTS] form focus: budget exhausted after N fields, no move` means the bare-form focus search hit its 300-field or 1 s cap and failed closed.
